...
function DoctorList() {
  ...
  useEffect(() => {
    if (location.state) {
      ...
      if (newTab) {
        console.log('Setting activeTab from location.state:', newTab);
        setActiveTab(newTab);
      }
    }
  }, [location.state]);
  ...
  const handleDownload = async (doc, recordName) => {
    ...
    try {
      ...
      const response = await fetch(
        `http://127.0.0.1:8000/api/dossier-medicale/dossiers/download/${doc.id}/`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
      ...
      <body>
        ...
        <div class="section">
          ...
          <div class="field"><span class="label">Date de naissance:</span> ${
            record.date_naissance || 'N/A'
          }</div>
          <div class="field"><span class="label">Lieu de naissance:</span> ${
            record.lieu_naissance || 'N/A'
          }</div>
          ...
          <div class="field"><span class="label">Numéro de téléphone:</span> ${
            record.numero_telephone || 'N/A'
          }</div>
          <div class="field"><span class="label">Email:</span> ${record.email || 'N/A'}</div>
          <div class="field"><span class="label">Situation familiale:</span> ${
            record.situation_familiale || 'N/A'
          }</div>
          <div class="field"><span class="label">Admis(e):</span> ${
            record.admission_etablissement || 'N/A'
          }</div>
          ...
          <div class="field"><span class="label">Numéro de dossier:</span> ${
            record.numero_dossier || 'N/A'
          }</div>
          <div class="field"><span class="label">Groupe sanguin:</span> ${
            record.groupe_sanguin || 'N/A'
          }</div>
          <div class="field"><span class="label">Numéro sécurité sociale:</span> ${
            record.numero_securite_sociale || 'N/A'
          }</div>
        </div>
        <div class="section">
          <h2>Données Biométriques</h2>
          <div class="field"><span class="label">Taille:</span> ${
            record.taille || 'N/A'
          } cm</div>
          <div class="field"><span class="label">Poids:</span> ${
            record.poids || 'N/A'
          } kg</div>
          <div class="field"><span class="label">Fréquence cardiaque:</span> ${
            record.frequence_cardiaque || 'N/A'
          } bpm</div>
          <div class="field"><span class="label">Pression artérielle:</span> ${
            record.pression_arterielle || 'N/A'
          }</div>
        </div>
        <div class="section">
          <h2>Antécédents Personnels</h2>
          <div class="field"><span class="label">Fumeur:</span> ${
            record.fumeur ? 'Oui' : 'Non'
          }</div>
          ${
            record.fumeur
              ? `<div class="field"><span class="label">Nombre de cigarettes:</span> ${
                  record.nombre_cigarettes || 'N/A'
                }</div>`
              : ''
          }
          <div class="field"><span class="label">Chiqueur:</span> ${
            record.chiqueur ? 'Oui' : 'Non'
          }</div>
          ${
            record.chiqueur
              ? `<div class="field"><span class="label">Nombre de boîtes chique:</span> ${
                  record.nombre_boites_chique || 'N/A'
                }</div>`
              : ''
          }
          <div class="field"><span class="label">Prise autre:</span> ${
            record.prise_autre ? 'Oui' : 'Non'
          }</div>
          ${
            record.prise_autre
              ? `<div class="field"><span class="label">Nombre de boîtes autre:</span> ${
                  record.nombre_boites_autre || 'N/A'
                }</div>`
              : ''
          }
          <div class="field"><span class="label">Âge première prise:</span> ${
            record.age_premiere_prise || 'N/A'
          }</div>
          <div class="field"><span class="label">Ancien fumeur:</span> ${
            record.ancien_fumeur ? 'Oui' : 'Non'
          }</div>
          ${
            record.ancien_fumeur
              ? `<div class="field"><span class="label">Nombre de boîtes fumeur:</span> ${
                  record.nombre_boites_fumeur || 'N/A'
                }</div>`
              : ''
          }
        </div>
        <div class="section">
          ...
          <div class="field"><span class="label">Affections congénitales:</span> ${
            record.affections_congenitales || 'N/A'
          }</div>
          <div class="field"><span class="label">Maladies générales:</span> ${
            record.maladies_generales || 'N/A'
          }</div>
          <div class="field"><span class="label">Interventions chirurgicales:</span> ${
            record.interventions_chirurgicales || 'N/A'
          }</div>
          <div class="field"><span class="label">Réactions allergiques:</span> ${
            record.reactions_allergiques || 'N/A'
          }</div>
        </div>
      </body>
      </html>
  ...
  return (
    <div className="app-container">
      <SideBareDocs activeTab={activeTab} setActiveTab={setActiveTab} /> {/* No disableDropdown */}
                ...
                <div key={record.id} className="record-card">
                  ...
                  <div className="record-actions">
                    ...
                    <BsFillSave2Fill
                      ...
                      onClick={() =>
                        handleDownload(
                          record.dossier_documents?.length > 0
                            ? record.dossier_documents[0]
                            : null,
                          `${record.nom} ${record.prenom}`
                        )
                      }
