...
function ModifyFormEnseignant() {
  ...
  // Fetch dossier data with corrected endpoint
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (!token) {
          alert('Aucun jeton d’authentification trouvé. Veuillez vous reconnecter.');
          navigate('/login');
          return;
        }
        const res = await fetch(`http://127.0.0.1:8000/api/dossier-medicale/dossiers/enseignants/${id}/`, {
          headers: {
        ...
        if (!res.ok) {
          ...
          if (res.status === 404) {
            alert(`Dossier avec ID ${id} non trouvé. Vérifiez l'ID ou créez un nouveau dossier.`);
          } else if (res.status === 401) {
            alert('Session expirée. Veuillez vous reconnecter.');
            navigate('/login');
          } else {
            alert(`Erreur lors du chargement: ${res.status} ${res.statusText}`);
          }
  ...
  const handlePressionArterielle = (e) => {
    ...
    if (/^[0-9]*\/?[0-9]*$/.test(value)) {
      ...
      setFieldErrors((prev) => ({ ...prev, pression_arterielle: '' }));
    }
  };
        ...
        {activeTab === 'infos' && (
          <div className="border rounded-lg shadow bg-white p-6 w-full h-full">
            ...
            <div className="grid grid-cols-2 gap-6 mb-4">
              ...
              <div>
                ...
                <input
                  type="text"
                  name="specialite"
                  className={`input-style ${fieldErrors.specialite ? 'input-error' : ''}`}
            ...
            <div className="grid grid-cols-2 gap-6">
              ...
              <div>
                <label className="block mb-1 font-medium">Admission Établissement</label>
                <select
                  name="admission_etablissement"
                  className={`input-style ${fieldErrors.admission_etablissement ? 'input-error' : ''}`}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => handleChangeDepistage('pression_oculaire', e.target.value)}
                      placeholder="ex: 15.5"
                    />
                  </div>
                  <div>
