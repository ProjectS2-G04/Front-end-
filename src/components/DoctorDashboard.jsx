...
import { FaHeartbeat, FaChartBar, FaExclamationTriangle } from 'react-icons/fa';
import {
  LineChart,
  Line,
  XAxis,
...
export default function DoctorDashboard() {
  ...
  useEffect(() => {
    if (role !== 'DOCTOR') {
      console.log('Unauthorized access, redirecting to /dashboarddirector');
  ...
  const fetchStats = async () => {
    try {
      const response = await axios.get('http://localhost:8000/statistics/api/dashboard-stats/', {
        params: { start_date: startDate, end_date: endDate }, // Fixed typo: endDate to end_date
      });
    ...
    <div className="flex overflow-x-hidden">
      ...
      <div className="ml-64 p-6 w-full">
        ...
        <div className="grid grid-cols-3 gap-4 mb-6">
          ...
          >
            <FaChartBar className="mx-auto text-2xl text-teal-500" />
