...
function ReadOnlyTeacher() {
   const [activeTab, setActiveTab] = useState("infos");
 const [nom, setNom] = useState("Benali");
const [prenom, setPrenom] = useState("Sofia");
const [dateNaissance, setDateNaissance] = useState("1995-08-15");
const [lieuNaissance, setWilaya] = useState("16"); 
const [adresse, setAdresse] = useState("12 rue des Fleurs, Alger");
const [email, setEmail] = useState("sofia.benali@example.com");
const [telephone, setTelephone] = useState("+213 551 234 567");
const [nss, setNss] = useState("1234567890123");
...
const [grade, setGrade] = useState("Médecin spécialiste");
...
<div className="relative w-32 h-32 rounded-full bg-white border-4 border-transparent bg-clip-padding p-1 shadow-lg"
 ...
 {activeTab === "infos" && (
  <div className="border rounded-lg shadow bg-white p-6 w-full h-full">
      ...
      <div className="grid grid-cols-2 gap-6 mb-4">
        <div>
          <label className="block mb-1 font-medium">Spécialitée</label>
          <p className="readonly-style">{specialite}</p>
        </div>
         <div>
...
export default ReadOnlyTeacher